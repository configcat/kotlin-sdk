...
class CommonUtilsTests {
    ...
    fun testValidateSettingValueType() = runTest {
        ...
        var result: Any = Utils.validateSettingValueType(settingValue, -1)
        assertEquals(3.14, result)
        result = Utils.validateSettingValueType(settingValue, 0)
        assertEquals(true, result)
        result = Utils.validateSettingValueType(settingValue, 1)
        assertEquals("stringValue", result)
        result = Utils.validateSettingValueType(settingValue, 2)
        assertEquals(1, result)
        result = Utils.validateSettingValueType(settingValue, 3)
        assertEquals(3.14, result)
        ...
        val invalidSettingResultException = assertFailsWith(
            ...
            block = { Utils.validateSettingValueType(invalidSettingValue, 1) }
        )
        ...
        val invalidTypeException = assertFailsWith(
            exceptionClass = IllegalArgumentException::class,
            block = { Utils.validateSettingValueType(settingValue, 99) }
        )
        ...
        val invalidSettingValueException = assertFailsWith(
            exceptionClass = IllegalArgumentException::class,
            block = { Utils.validateSettingValueType(null, 1) }
        )
    ...
    fun testParseConfigJson() = runTest {
        val configResult = Utils.parseConfigJson(Data.formatJsonBodyWithString("fake"))
